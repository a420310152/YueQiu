...
public class ResponseChallengeActivity extends Activity {
    ...
    private static Logx logx = new Logx(ResponseChallengeActivity.class);
    ...
    View.OnClickListener clickOk = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (person.getObjectId().equals(challenge.getInitiator().getObjectId())) {
                ...
                MyChallege.query(challenge, new FindListener<MyChallege>() {
                    @Override
                    public void onSuccess(List<MyChallege> list) {
                        for (MyChallege i : list) {
                            if (Utils.equals(i.getPerson().getObjectId(), person.getObjectId())) {
                                logx.toast("您已经报名，不要重复报名哦！");
                                return;
                            }
                        }

                        MyChallege.insert(challenge, person, new SaveListener() {
                            @Override
                            public void onSuccess() {
                                logx.toast("报名成功！请您准时赴约哦！");
                            }
    ...
    View.OnClickListener noClick = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            MyChallege.delete(challenge, person);
            logx.toast("取消报名成功");
            finish();
        }
    };
    ...
    private void buildResponse(){
        ...
        MyChallege.query(challenge, new FindListener<MyChallege>() {
            ...
            public void onSuccess(List<MyChallege> list) {
                personList.clear();
                for (MyChallege i : list) {
                    personList.add(i.getPerson());
                    logx.a(i.getPerson() != null, "Person 为空");
                }
                logx.e("personList大小为" + personList.size());
                mAdapter = new GalleryAdapter(context, personList);
                mRecyclerView.setAdapter(mAdapter);
            }

            @Override
            public void onError(int i, String s) {
                logx.e("查询响应者失败 i=" + i + ", s=" + s);
            }
        });
    }
}
